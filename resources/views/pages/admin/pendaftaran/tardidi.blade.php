						...
						<table class="table datatable-fixed-right">
							...
							<tbody>
								...
								<tr>
									...
									<td class="text-capitalize">{{$item->tempat_lahir}} , {{ \Carbon\Carbon::parse($item->tanggal_lahir)->isoFormat('D MMMM Y') }}</td>
									<td class="text-capitalize">{{$item->pendeta}}</td>
									<td class="text-capitalize">{{$item->guru_jemaat}}</td>
									<td class="text-capitalize">{{$item->ayah}}</td>
									<td class="text-capitalize">{{$item->ibu}}</td>
									<td class="text-capitalize">{{ \Carbon\Carbon::parse($item->tanggal_lahir)->isoFormat('D MMMM Y') }}</td>
									<td class="text-center">
										<div class="d-inline-flex ">
											@if (Auth::user()->hasrole('admin'))
											<form action="{{route('admin.tardidi.destroy', $item->id)}}" method="POST">
											@else
											<form action="{{route('sek.tardidi.destroy', $item->id)}}" method="POST">												
											@endif
												@csrf
												@method('DELETE')
												@if(is_null($item->no_surat) || is_null($item->nama) || is_null($item->id_sektor) || is_null($item->tempat_lahir) || is_null($item->tanggal_lahir) || is_null($item->tanggal_baptis) || is_null($item->ayah) || is_null($item->ibu) || is_null($item->pendeta) || is_null($item->guru_jemaat) )
												
												@else
												<a type="button" method="GET" href="{{route('tardidi_pdf', $item->id)}}" data-bs-popup="tooltip" title="Unduh" class="btn btn-secondary btn-icon" style="margin-bottom: -1rem">
													<i class="ph-file-pdf"></i>
												</a>	
												
												@endif
												@if (Auth::user()->hasrole('admin'))
												<a type="button" method="GET" href="{{route('admin.tardidi.edit', $item->id)}}" data-bs-popup="tooltip" title="Edit" class="btn btn-warning btn-icon" style="margin-bottom: -1rem">
													<i class="ph-note-pencil"></i>
												</a>
												@else
												<a type="button" method="GET" href="{{route('sek.tardidi.edit', $item->id)}}" data-bs-popup="tooltip" title="Edit" class="btn btn-warning btn-icon" style="margin-bottom: -1rem">
													<i class="ph-note-pencil"></i>
												</a>													
												@endif
