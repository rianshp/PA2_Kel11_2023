						...
						<table class="table datatable-fixed-right">
							...
							<tbody>
								...
								<tr>
									...
									<td class="text-capitalize">{{ \Carbon\Carbon::parse($item->tanggal_sidi)->isoFormat('D MMMM Y') }}</td>									
									<td class="text-capitalize">{{$item->nats}}</td>
									<td class="text-capitalize">{{$item->pendeta}}</td>
									<td class="text-capitalize">{{$item->guru_jemaat}}</td>									
									<td class="text-center">
										<div class="d-inline-flex ">
											@if (Auth::user()->hasrole('admin'))
											<form action="{{route('admin.malua.destroy', $item->id)}}" method="POST">
											@else
											<form action="{{route('sek.malua.destroy', $item->id)}}" method="POST">												
											@endif
												@csrf
												@method('DELETE')
												@if(is_null($item->no_surat) || is_null($item->id_user_jemaat) || is_null($item->tempat_lahir) || is_null($item->tanggal_lahir) || is_null($item->tanggal_baptis)|| is_null($item->tanggal_sidi)  || is_null($item->nats) || is_null($item->pendeta) || is_null($item->guru_jemaat))												
												@else												
												<a type="button" method="GET" href="{{route('malua_pdf', $item->id)}}" data-bs-popup="tooltip" title="Unduh" class="btn btn-secondary btn-icon" style="margin-bottom: -1rem">
													<i class="ph-file-pdf"></i>
												</a>	
												@endif
												@if (Auth::user()->hasrole('admin'))
												<a type="button" method="GET" href="{{route('admin.malua.edit', $item->id)}}" data-bs-popup="tooltip" title="Edit" class="btn btn-warning btn-icon" style="margin-bottom: -1rem">
													<i class="ph-note-pencil"></i>
												</a>
												@else
												<a type="button" method="GET" href="{{route('sek.malua.edit', $item->id)}}" data-bs-popup="tooltip" title="Edit" class="btn btn-warning btn-icon" style="margin-bottom: -1rem">
													<i class="ph-note-pencil"></i>
												</a>													
												@endif
