							...
							<tbody>
								...
								<tr>
									...
									<td class="text-capitalize">{{$k->pindah_ke}}</td>
											...
											<form action="{{route('keluarga.destroy', $k->id)}}" method="POST">
												@csrf
												@method('DELETE')
												<a type="button" method="GET" href="{{route('keluarga.edit', $k->id)}}" data-bs-popup="tooltip" title="Edit" class="btn btn-warning btn-icon" style="margin-bottom: -1rem">
