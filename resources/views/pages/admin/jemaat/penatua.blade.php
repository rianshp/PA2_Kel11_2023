						...
						<table class="table datatable-basic">
							...
							<tbody>
								...
								<tr>
										...
										<td class="text-capitalize">{{$item->penatua->sektor->nama}}</td>
										<td>{{ \Carbon\Carbon::parse($item->tgl_lahir)->age}} Tahun</td>
										<td class="text-capitalize">{{$item->penatua->status}}</td>
										<td class="text-center">
											<div class="d-inline-flex ">
												<form action="{{route('penatua.destroy', $item->id)}}" method="POST">
													@csrf
													@method('DELETE')
