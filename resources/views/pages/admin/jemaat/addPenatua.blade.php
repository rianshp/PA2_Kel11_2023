                     ...
                     <div class="page-header-content d-lg-flex border-top py-1">
                            <div class="d-flex">
								...
								@endhasrole
                                          ...
                                          <span class="breadcrumb-item active">Tambah Data penatua</span>
                                   </div>
              ...
              <div class="content">
                     ...
                     <div class="card">
                         <div class="card-header">
                              @if ($penatua->id)
                              <h5 class="mb-0">Edit Penatua</h5>    
                              @else
                                  
                              <h5 class="mb-0">Tambah Penatua</h5>
                              @endif
                         </div>                        
                         ...
                         <div class="card-body border-top">
                              @if ($penatua->id)
                                   ...
                                   @else
                                   <form action="{{route('sek.penatua.update', $penatua->id)}}" method="POST">                                            
                                   @endif
                                        @csrf                                                   
                                        @method('PATCH')          
                                        <div class="mb-3">
                                             ...
                                             <div class="form-control-feedback">
                                                  <select name="id_jemaat" class="form-control select @error('id_jemaat') is-invalid @enderror">
                                                            <option value="" selected disabled>Nama Jemaat</option>
                                                            @foreach ($jemaat as $j)
                                                                 <option value="{{$j->id}}" {{$j->id==$penatua->id_jemaat?"selected":""}}>{{$j->nama}}</option>
                                                            @endforeach                                             
                                                  </select>
                                                  @error('id_jemaat')                                                               
                                        ...
                                        <div class="mb-3">
                                             <label class="form-label">Status <span class="text-danger">*</span></label>
                                                  <select name="status" class="form-control form-control-select2 @error('status') is-invalid @enderror">                                                     
                                                       <option value="aktif" {{$penatua->status == "aktif"?"selected":""}}>Aktif</option>    
                                                       <option value="non-aktif" {{$penatua->status == "non-aktif"?"selected":""}}>Non-Aktif</option>                                             
                                                  </select>
                                                  @error('status')                                                               
                                                  <div class="invalid-feedback">{{$message}}</div>
                                                       <script>
                                        ...
                                        <div class="mb-3">
                                             ...
                                             <div class="form-control-feedback">
                                                  <select name="id_sektor" class="form-control select @error('id_sektor') is-invalid @enderror">
                                                            <option value="" selected disabled>Pilih Sektor</option>
                                                            @foreach ($sektor as $s)
                                                                 <option value="{{$s->id}}" {{$s->id==$penatua->id_sektor?"selected":""}}>{{$s->nama}}</option>
                                                            @endforeach                                             
                                                  </select>
                                                  @error('id_sektor')                                                               
                                                  <div class="invalid-feedback">{{$message}}</div>
                                        ...
                                        <div class="text-end">
                                             <button type="submit" class="btn btn-primary">Tambah Penatua <i class="ph-paper-plane-tilt ms-2"></i></button>
                                   </div>

                                   </form>
                              @else          
                                   @if (Auth::user()->hasrole('admin'))
                                   <form action="{{route('admin.penatua.store')}}" method="POST">              
                                   @else
                                   <form action="{{route('sek.penatua.store')}}" method="POST">                                                 
                                   @endif                        
                                        @csrf                                                             
                                        <div class="mb-3">
                                             <label class="col-form-label col-lg-3">Nama <span class="text-danger">*</span></label>
                                        ...
                                        <div class="mb-3">
                                             <label class="form-label">Gelar </label>
                                             <input type="text" name="gelar" class="form-control @error('sektor') is-invalid @enderror" placeholder="Gelar">
                                        </div>
                                        <div class="text-end">
                                             <button type="submit" class="btn btn-primary">Tambah Penatua <i class="ph-paper-plane-tilt ms-2"></i></button>
                                   </div>
